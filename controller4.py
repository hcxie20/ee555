...
DEFAULT_GATEWAY = 1
validIP = [
  IPAddr('10.0.1.1'), IPAddr('10.0.1.2'), IPAddr('10.0.1.3'), IPAddr('10.0.1.4'), 
  IPAddr('10.0.2.1'), IPAddr('10.0.2.2'), IPAddr('10.0.2.3'), IPAddr('10.0.2.4'), 
  IPAddr('10.0.3.1'), IPAddr('10.0.3.2'), IPAddr('10.0.3.3'), IPAddr('10.0.3.4'),
  ]
subnet1 = ['10.0.1.1', '10.0.1.2', '10.0.1.3', '10.0.1.4']
subnet2 = ['10.0.2.1', '10.0.2.2', '10.0.2.3', '10.0.2.4']
subnet3 = ['10.0.3.1', '10.0.3.2', '10.0.3.3', '10.0.3.4']
...
class router(object):
  ...
  def _handle_PacketIn (self, event):
    ...
    if isinstance(payload, arp):
      ...
      if payload.dstip == self.routerIP[dpid]:
      ...
      elif (payload.dstip in subnet1 and self.routerIP[dpid] not in subnet1) or (payload.dstip in subnet2 and self.routerIP[dpid] not in subnet2) or (payload.dstip in subnet3 and self.routerIP[dpid] not in subnet3):
        #different subnet
        nextIP = IPAddr(self._find_gatewayIP(payload.dstip))
        log.debug("Different subnet. Next ip is {0}".format(nextIP))
        nextMac = self.arpTable[dpid][nextIP]
        port = self.routingTable[dpid][nextIP]
        log.debug(self.routingTable[dpid])
        ...
        msg.actions.append(of.ofp_action_output(port=port))# FIXED router to router port
        self.connections[dpid].send(msg)
        log.debug('Router {0}: packet from {1} to {2}, is in different subnet, send to port {3}'.format(dpid, payload.srcip, payload.dstip, port))
        ...
        if payload.dstip not in self.routingTable[dpid] or payload.dstip not in self.arpTable[dpid]:
          ...
          self._handle_ARP_Request(self.routerIP[dpid], payload.dstip, packet.src, of.OFPP_FLOOD, dpid)
        else:
          msg = of.ofp_packet_out(buffer_id=packetIn.buffer_id, in_port=inport)
  ...
  def _handle_ARP(self, ARP_packet, inport, dpid, packetIn):
    ...
    if ARP_packet.opcode == arp.REQUEST:
      ...
      if ARP_packet.protodst == self.routerIP[dpid]:
        ...
        if ARP_packet.protodst in subnets[dpid]:
          # packet for my subnet, flooding
          log.debug("Router {0}: ARP Request, for my subnet, flooding".format(dpid))
          self._send_packet(dpid, packetIn, of.OFPP_FLOOD)
          pass
        else:
          log.debug("Router {0}: ARP Request, not for me or my subnet".format(dpid))
          # self._send_packet(dpid, packetIn, of.OFPP_FLOOD)
  ...
  def _find_gatewayIP(self, ip):
    for i in range(3):
      if ip in subnets[i+1]:
        return subnets[i+1][0]
